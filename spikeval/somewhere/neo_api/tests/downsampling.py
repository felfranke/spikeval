...
if __name__ == "__main__":
    for l in lengths:
        ...
        for s in scales:
            a = np.random.rand(l)
            print "Scale %d" % s
            t = timeit.Timer('sci_down(a, %d)' % s,
                             'from __main__ import sci_down, a')
            elapsed = (
            1000 * t.timeit(number=10) / 10) # result should be in ms
            print "Using DOWNsampling takes %0.3f milliseconds/pass" % elapsed
            t = timeit.Timer('sci_resa(a, %d)' % s,
                             'from __main__ import sci_resa, a')
            elapsed = (
            1000 * t.timeit(number=10) / 10) # result should be in ms

from meta import meta_attributes, meta_data_attrs, meta_children, \
    meta_parents, allowed_range_params
from datetime import datetime

def clean_attr(_attr):
    """
    By default attribute names in meta contain prefix "_" to indicate
    whether an
    attribute is mandatory. This needs to be cleaned up before assigning to
    the
    NEO object.
    """
...
def json_builder(obj, GET={}, enquery="full"):
    ...
    n["neo_id"] = obj.neo_id
    """ this variable is used to track whether at least some information
    exists
    for the requested object. This is needed to raise an error when
    knowingly a
    ...
    if enquery == "data" or enquery == "full":
        ...
        for k, v in GET.items():
            if k in allowed_range_params.keys() and allowed_range_params.get(
                k)(v):
...
def assign_data_arrays(json, obj, **params):
    """
    Assigns data-related attrs from NEO to json object for later HTTP
    response.
    """
    assigned = False
...
def assign_common(json, obj):
    """
    Assigns common information from NEO to json object for later HTTP
    response.
    """

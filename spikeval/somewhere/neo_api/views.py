...
class BasicJSONResponse(HttpResponse):
    ...
    will pickle a response. Sets up an appropriate Cont-Type. User and
    Message
    always appreciated.
    """
...
def parse_neo_id(neo_id):
    ...
    if mid > 0 and len(neo_id) > mid + 1: # exclude error in case of
    # "segment_"
...
def get_object(obj_type, obj_id, user):
    ...
    Attention! This function returns HTTP response in case an exception
    occurs.
    """
...
def handle_object(request, obj_type, obj_id):
    ...
    if request.method in actions.keys():
        ...
        response = NotSupported(message_type="invalid_method",
                                request=request)
    return response
...
def create_or_update(request, obj_type, obj=None):
    ...
    everything "that's not made by our hands" for security reasons (no
    automatic
    ...
    to_link, parents = None, None # handlers for special cases (waveforms,
    # units)
    try:
    ...
    for _attr in meta_attributes[obj_type]:
        ...
        if rdata.has_key(attr):
            ...
            return BadRequest(json_obj={"obj_type":attr},\
                              message_type="missing_parameter",
                              request=request)
    if not update:
        ...
        for data_attr in meta_data_attrs[obj_type]:
            if rdata.has_key(data_attr):
                if data_attr == "waveforms":
                    # Waveforms - it's a special case,
                    # 2-3D array. Parsing and update is made of
                    # three stages: we create new waveforms first (no save),
                    # then delete old
                    # ones, then save and assign new waveforms to the host
                    # object.
                    waveforms = rdata[
                                data_attr] # some processing is done later
                                # in this view
                    if not getattr(waveforms, "__iter__", False):
                        return BadRequest(message_type="not_iterable",
                                          request=request)
                    to_link = [] # later this list is used to link waveforms
                    # to the obj
                    for wf in waveforms:
                        try:
                            w = WaveForm()
                            try:
                                ...
                                if obj_type == "spiketrain":
                                    w.time_of_spike_data = \
                                    wf["time_of_spike"][
                                                           "data"]
                                    w.time_of_spike__unit =
                                    wf["time_of_spike"]["units"]
                                to_link.append(w)
                            except KeyError:
                                ...
                                return BadRequest(
                                    json_obj={"details":v.message},\
                                    message_type="bad_float_data",
                                    request=request)
                        except AttributeError, TypeError:
                            return BadRequest(json_obj={"element":data_attr},\
                                              message_type="dict_required",
                                              request=request)
                else:
    ...
    if meta_parents.has_key(obj_type):
        if obj_type == "unit":
            # unit is a special case. there may be several parents in one
            # parameter.
            ...
            if rdata.has_key(r):
                ...
                for p in parent_ids: # this looks ugly.
                    ...
                    if isinstance(parent, HttpResponse):
                        return parent
                    parents.append(
                        parent) # some processing done later in this view
        else:
    ...
    # this save() and delete() operations can be earliest done here,
    # to exclude
...
def get_single(request, obj_type, obj, message_type="retrieved", new=False):
    """
    This is a slave function to get a single NEO object by given NEO_ID. Due
     to
    ...
    if new: return Created(json_obj=n, message_type=message_type,
                           request=request)
    return BasicJSONResponse(json_obj=n, message_type=message_type,
                             request=request)
